...
u8 BLTD_GetRecievedData(u8 Data[], u16 Length)
{
    ...
    {
        IsRespRecieved = 0u;
        RespStatus11 = BLTD_RESP_STATUS_OK;
        for( i = 0u; i< Length ; i++)
        {
            Data[i] = RxBuffer[i];
        }
    ...
    {
        RespStatus11 = BLTD_RESP_STATUS_NON;
    }
...
static void BTCommandSend(const u8* Command,u16 CommandLength)
    ...
    BTCommandBuffer[0] = 0x41u;
        BTCommandBuffer[1] = 0x54u;
