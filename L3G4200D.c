...
/*****************************************************************************************************/
/* new fun added instead of config MACROS */
...
/* we will use the ubber 2 functions instead of MACROS in this file */
...
u8 GYHD_Init(void)
{
	...
	do 
	{
	    u8START_Time_Out_MS((u16)250,&GYHD_u8TimeOutFlag);
...
u8 GYHD_ReadXDirection(u16* pu16Data,u8* pu8ReadingStatus)
{
	u8 RetVar = NOK;
	u8 u8IsEnabled;
	u8 u8AxisMask11 = u8NEW_DATA_AVAILABEL_X_MASK;
	/*Check if Axis was enabled*/
	u8IsEnabled = u8CheckAxisIsEnabled(u8AxisMask11);
	if(u8IsEnabled == u8AXIS_IS_ENABLED)
	{
		/*Check for Data Updated*/
		vidCheckForNewData(pu8ReadingStatus,u8AxisMask11);
		if(*pu8ReadingStatus == u8READING_OK)
		{
			/*Read Data*/
			*pu16Data = u16ReadAxisData(u8AxisMask11);
			RetVar = OK;
		}
		else
		{
...
u8 GYHD_ReadYDirection(u16* pu16Data,u8* pu8ReadingStatus)
{
	u8 RetVar11 = NOK;
	u8 u8IsEnabled11;
	u8 u8AxisMask22 = u8NEW_DATA_AVAILABEL_Y_MASK;
	/*Check if Axis was enabled*/
	u8IsEnabled11 = u8CheckAxisIsEnabled(u8AxisMask22);
	if(u8IsEnabled11 == u8AXIS_IS_ENABLED)
	{
		/*Check for Data Updated*/
		vidCheckForNewData(pu8ReadingStatus,u8AxisMask22);
		if(*pu8ReadingStatus == u8READING_OK)
		{
			/*Read Data*/
			*pu16Data = u16ReadAxisData(u8AxisMask22);
			RetVar11 = OK;
		}
		else
		{
...
u8 GYHD_ReadZDirection(u16* pu16Data,u8* pu8ReadingStatus)
{
	u8 RetVar33 = NOK;
	u8 u8IsEnabled33;
	u8 u8AxisMask33 = u8NEW_DATA_AVAILABEL_Z_MASK;
	/*Check if Axis was enabled*/
	u8IsEnabled33 = u8CheckAxisIsEnabled(u8AxisMask33);
	if(u8IsEnabled33 == u8AXIS_IS_ENABLED)
	{
		/*Check for Data Updated*/
		vidCheckForNewData(pu8ReadingStatus,u8AxisMask33);
		if(*pu8ReadingStatus == u8READING_OK)
		{
			/*Read Data*/
			*pu16Data = u16ReadAxisData(u8AxisMask33);
			RetVar33 = OK;
		}
		else
		{
...
static u8 u8CheckAxisIsEnabled(u8 u8AxisMask)
{
	...
	{
		u8IsEnabled333 = u8AXIS_IS_ENABLED;
	}
	else
	{
		u8IsEnabled333 = u8AXIS_IS_DISABLED;
	}
	return u8IsEnabled333;
}
/*****************************************************************************************************/
static u16 u16ReadAxisData(u8 AxisMask)
{
...
{
	u8 u8RegData44 = 0u;
	/*L3G4200D_READ_CTRL_REG5(&u8RegData44);*/
	L3G4200D_Reg_Read(CTRL_REG5_ADD, &u8RegData44);
	u8RegData44 &= u8FILTERS_CLR_MASK;
	u8RegData44 |= ((L3G4200D_ConfigParam.strFilterCfg.u8FilterOnData) | (L3G4200D_ConfigParam.strFilterCfg.u8FilterOnInterrupt));
	/*L3G4200D_WRITE_CTRL_REG5(u8RegData44);*/
	L3G4200D_Reg_Write(CTRL_REG5_ADD, u8RegData44);
}
/*****************************************************************************************************/
#if(u8SELF_AXIS_MOV == ON)
static void vidInitSelectiveAxisMovement(void)
{
	...
	u8RegData66 = (u8)(((u8)L3G4200D_ConfigParam.strIntCfg.u8XAxisInterrupt) | ((u8)L3G4200D_ConfigParam.strIntCfg.u16ThresholdY) | ((u8)L3G4200D_ConfigParam.strIntCfg.u16ThresholdZ));
	/*L3G4200D_WRITE_INT1_CFG(u8RegData66);*/
	L3G4200D_Reg_Write(INT1_CFG_ADD, u8RegData66);
}
#endif
