...
static u8  BLMGR_TxDeviceNameLength = 0u;
...
static void UpdateIdFrame(void)
{
    ...
    /*Set paramter length*/
    BLMGR_DataTxBuffer[PARAM_LENGTH_IDX] = (BLMGR_TxDeviceNameLength + 2u);
    /*Update Os Type*/
            ...
            if(BLMGR_DataRxBuffer[FRAME_TYPE_IDX] == FRAME_TYPE_VAL_FRAME)
            {
                ...
                (BLMGR_DataRxBuffer[FRAME_CRC_IDX] == TX_CRC_DEFAULT))
                {
                    ...
                    if(BLMGR_DataRxBuffer[FRAME_RECVER_IDX] == DEVICE_ROLE)
                    {
                        #if(COMM_CINFIG == MSTER_COMM)
                        /*Validate CRC */
                        IsFrameValid2 = CheckCrc();


                        #elif(COMM_CINFIG == SLAVE_COMM)
                        /*Get the Crc Key*/
                        IsFrameValid2 = GetCrcKey();
                        #endif
                    }
                    else
                    {
...
{
    static u8  BLMGR_TxBattLevel11;
    BLMGR_TxBattLevel11= BattLevel;
}

static void UpdateDataFrame(void)
{
    static u8  BLMGR_TxBattLevel;
    static u8 TempBuffer22[MAX_DATA_BUFFER_LENGTH];
    u16 Crc11 = 0u;
    /*Set Tx Frame to default values*/
    MemSet(&BLMGR_DataTxBuffer[FRAME_HEADER_IDX],TX_FRAME_DEFUALT,MAX_DATA_BUFFER_LENGTH);
    /*Set Header of Frame*/
    ...
    /*Calculate CRC*/
    MemCpy(TempBuffer22,&BLMGR_DataTxBuffer[BATT_LEVEL_IDX],1u);
    SECR_GnerateCrc(TempBuffer22,1u, &Crc11,BLMGR_CrcKey);
    #elif(COMM_CINFIG == SLAVE_COMM)
    /*Set paramter length*/
    ...
    /*Calculate CRC*/
    MemCpy(TempBuffer22,&BLMGR_DataTxBuffer[DIRECTION_IDX],2);
    SECR_GnerateCrc(TempBuffer22,2, &Crc11,BLMGR_CrcKey);
    #else
    /*Wrong Config, State in Idle*/
    /*To do: Managing dev Errors*/
    #endif
    /*Update Crc*/
    BLMGR_DataTxBuffer[FRAME_CRC_IDX] = (u8)Crc11;
    BLMGR_DataTxBuffer[FRAME_CRC_IDX + 1u] = (u8)(Crc11 >> 8u);
    /*update Frame CheckSum*/
    BLMGR_DataTxBuffer[FRAME_CHECKSUM_IDX] = CalculateCheksum(BLMGR_DataTxBuffer,FRAME_CHECKSUM_IDX -1u);
    /*update frame tail*/
    MemSet(&BLMGR_DataTxBuffer[FRAME_TAIL_IDX],0x55u,1u);
...
static u8 CheckDataFrame(void)
{
    ...
    static u8 TempBuffer11[MAX_DATA_BUFFER_LENGTH] = {0};
    u8 IsFrameValid6;
    u8 TempVar1;
    u16 GenCrc = 0u;
    u16 RecvdCrc;
    ...
    if (TempVar1 == BLMGR_DataRxBuffer[FRAME_CHECKSUM_IDX])
    {
        ...
        (BLMGR_DataRxBuffer[FRAME_TAIL_IDX] == 0x55u))
        {
            ...
            if(BLMGR_DataRxBuffer[FRAME_TYPE_IDX] == FRAME_TYPE_DATA_FRAME)
            {
                ...
                #if(COMM_CINFIG == MSTER_COMM)
                TempBuffer11[0x00] = BLMGR_DataRxBuffer[DIRECTION_IDX];
                TempBuffer11[0x01] = BLMGR_DataRxBuffer[SPEED_DEGREE_IDX];
                SECR_GnerateCrc(TempBuffer11, 2u, &GenCrc,BLMGR_CrcKey);
                #elif(COMM_CINFIG == SLAVE_COMM)
                TempBuffer11[0x00] = BLMGR_DataRxBuffer[BATT_LEVEL_IDX];
                SECR_GnerateCrc(TempBuffer11, 1, &GenCrc,BLMGR_CrcKey);
                #else
                /*Wrong Config, State in Idle*/
                /*To do: Managing dev Errors*/
                #endif
...
{
    u8 ErrorType11;
    ErrorType11 = BLMGR_DataRxBuffer[ERROR_TYPE_IDX];
    switch(ErrorType11)
    {
...
{
    u16 LoopIndex11;
    for(LoopIndex11 = 0u; LoopIndex11 < Length; LoopIndex11 ++)
    {
        DesPtr[LoopIndex11] = SrcPtr[LoopIndex11];
    }
}
/*********************************************************************************/
static void MemSet(u8 DesPtr[], u8 ConstVal, u16 Length)
{
    u16 LoopIndex22;
    for(LoopIndex22 = 0u; LoopIndex22 < Length; LoopIndex22 ++)
    {
        DesPtr[LoopIndex22] = ConstVal;
    }
}
/*********************************************************************************/
#if (COMM_CINFIG == SLAVE_COMM)
static u8 MemCmp(const u8* Src1Ptr,const u8* Src2Ptr,u16 Length)
{
    u8 IsEqual = 1u;
    u8 LoopIndex;
    for (LoopIndex = 0u; (LoopIndex < Length) && (IsEqual == 1) ; LoopIndex ++)
    {
...
static u8 CalculateCheksum(const u8 BufferPtr[], u16 BufferLength)
{
    u32 Checksum = 0x00u;
    u16 LoopIndex33;
    for (LoopIndex33 = 0u; LoopIndex33 <= BufferLength; LoopIndex33 ++)
    {
        Checksum += BufferPtr[LoopIndex33];
    }
    Checksum %= 256u;
    return (u8)Checksum;
}
...
static u8 CheckCrc(void)
{
    u16 RxCrc;
    u16 GenCrc33 = 0u;
    u8 TempBuffer44[MAX_DATA_BUFFER_LENGTH];
    u8 IsFrameValid66;
    RxCrc = 0x00u;
    RxCrc = BLMGR_DataRxBuffer[FRAME_VAL_CRC_IDX];
    RxCrc |= (u16)(((u16)BLMGR_DataRxBuffer[FRAME_VAL_CRC_IDX +1u]) << (8u));
    TempBuffer44[0x00] = TX_OS_TYPE;
    TempBuffer44[0x01] = TX_DEV_TYPE;
    MemCpy(&TempBuffer44[0x02u],BLMGR_TxDevicName, (u16)BLMGR_TxDeviceNameLength);
    BLMGR_TxDeviceNameLength += 2u;
    SECR_GnerateCrc(TempBuffer44, (u16)BLMGR_TxDeviceNameLength, &GenCrc33, BLMGR_CrcKey);
    if(GenCrc33 == RxCrc)
    {
        BLMGR_ErrorState = ERRH_NO_ERROR;
        IsFrameValid66 = 1u;
...
static void BuzzerInit(void)
{
    DIO_InitPortDirection(BuzzerConfig.Portname,0xffu,BuzzerConfig.Port_Mask);
    DIO_WritePort(BuzzerConfig.Portname,0x00u,BuzzerConfig.Port_Mask);
}
/*********************************************************************************/
static void PowerBlueToothInit(void)
{
    DIO_InitPortDirection(BlueToothPwrConfig.Portname,0xffu,BlueToothPwrConfig.Port_Mask);
    DIO_WritePort(BlueToothPwrConfig.Portname,0x00u,BlueToothPwrConfig.Port_Mask);
}
/*********************************************************************************/
static void BlueToothKeyInit(void)
{
    DIO_InitPortDirection(BluetoothKeyConfig.Portname,0xffu,BluetoothKeyConfig.Port_Mask);
    DIO_WritePort(BluetoothKeyConfig.Portname,0xffu,BluetoothKeyConfig.Port_Mask);
}

static void InserBreakPoint(void)
{
    DIO_WritePort(BuzzerConfig.Portname,0xffu,BuzzerConfig.Port_Mask);
    while(1)
    {
